...
export { EmbedResolver } from './embed.js';
...
export function enrichMediaUrls(data) {
  if (!data || typeof data !== 'object') return data;

  if (Array.isArray(data)) {
    return data.map(item => enrichMediaUrls(item));
  }
  ...
  for (const [key, val] of Object.entries(data)) {
    if (val && typeof val === 'object') {
      enriched[key] = enrichMediaUrls(val);
      continue;
    }
    ...
    if (typeof val === 'string' && val.length > 0) {
      ...
      if (cleanVal.startsWith('/')) {
        if (key === 'poster_path') {
          enriched.poster_url = `${TMDB_IMAGE_BASE}/w500${cleanVal}`;
          enriched.poster_url_original = `${TMDB_IMAGE_BASE}/original${cleanVal}`;
        } else if (key === 'backdrop_path') {
          enriched.backdrop_url = `${TMDB_IMAGE_BASE}/w1280${cleanVal}`;
          enriched.backdrop_url_original = `${TMDB_IMAGE_BASE}/original${cleanVal}`;
        } else if (key === 'profile_path') {
          enriched.profile_url = `${TMDB_IMAGE_BASE}/w185${cleanVal}`;
          enriched.profile_url_original = `${TMDB_IMAGE_BASE}/original${cleanVal}`;
        } else if (key === 'logo_path') {
          enriched.logo_url = `${TMDB_IMAGE_BASE}/w500${cleanVal}`;
          enriched.logo_url_original = `${TMDB_IMAGE_BASE}/original${cleanVal}`;
        } else if (key === 'still_path') {
          enriched.still_url = `${TMDB_IMAGE_BASE}/w300${cleanVal}`;
          enriched.still_url_original = `${TMDB_IMAGE_BASE}/original${cleanVal}`;
        } else if (key === 'file_path') {
          enriched.file_url = `${TMDB_IMAGE_BASE}/original${cleanVal}`;
          enriched.file_url_w500 = `${TMDB_IMAGE_BASE}/w500${cleanVal}`;
        } else if (key.endsWith('_path')) {
          const prefix = key.replace(/_path$/, '');
          enriched[`${prefix}_url`] = `${TMDB_IMAGE_BASE}/original${cleanVal}`;
        }
      }
    }
  }
...
export class OneFlex {
  constructor(options = {}) {
    ...
    this.embedResolver = new EmbedResolver({ userAgent: this.userAgent, timeout: this.timeout });
  }
  ...
  async fetchFromTMDB(endpoint, params = {}) {
    ...
    const enrichedData = enrichMediaUrls(data);
  ...
  async getStreamSources(options = {}) {
    ...
    if (resolve) {
      try {
        const resolved = await this.embedResolver.resolveDirectStreams({
          id,
          type: isMovie ? 'movie' : 'tv',
          season,
          episode
        });
        directStreams = resolved.streams || [];
        directSubtitles = resolved.subtitles || [];
      } catch {}
    }
  ...
  async resolveStreams(options = {}) {
    return this.embedResolver.resolveDirectStreams(options);
  }

  async resolveEmbed(embedUrl) {
    return this.embedResolver.resolveEmbedUrl(embedUrl);
  }
  ...
  async getMatches(filter = 'live', popular = false) {
    ...
    return matches.map(match => ({
      ...
      badge_home_url: match.teams?.home?.badge ? `${this.dbUrl}/api/sports/images/badge/${match.teams.home.badge}.webp` : null,
      badge_away_url: match.teams?.away?.badge ? `${this.dbUrl}/api/sports/images/badge/${match.teams.away.badge}.webp` : null,
      poster_url: match.poster ? `${this.dbUrl}${match.poster}` : null
    }));
  }
  ...
  async getGames(query = null, tag = null) {
    ...
    let games = raw.map(g => ({
      ...
      thumbnail_url: `${GAMES_THUMB_URL}/${g.directory}/${g.image}`,
      fallback_thumbnail_url: `${GAMES_ASSETS_URL}/${g.directory}/${g.image}`,
      play_url: `${GAMES_ASSETS_URL}/${g.directory}/index.html`,
      image_url: `${GAMES_THUMB_URL}/${g.directory}/${g.image}`
  ...
  async getUserWatchlist(userId) {
    if (!userId) throw new Error('userId is required');
    const data = await this.request(`${this.baseUrl}/api/lists/get-user-lists?userId=${encodeURIComponent(userId)}`);
    return enrichMediaUrls(data);
  }
  ...
  async getUserRatings(userId) {
    if (!userId) throw new Error('userId is required');
    const data = await this.request(`${this.baseUrl}/api/ratings/get-user-ratings?userId=${encodeURIComponent(userId)}`);
    return enrichMediaUrls(data);
  }
  ...
  async healthCheck() {
    const tests = [
      ...
      {
        service: 'Direct M3U8 Stream Resolver',
        fn: () => this.resolveStreams({ id: 550, type: 'movie' })
      },
      {
